...
@bot.on(xubot_cmd(outgoing=True, pattern="getid(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(xubot_cmd(outgoing=True, pattern="link(?: |$)(.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="getbot(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"logit(?: |$)([\s\S]*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="kickme$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="mutechat$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="regexninja (on|off)$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="chatinfo(?: |$)(.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="invite(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
\n\n`{xcm}link` <username/userid> : <optional text> (or) reply to someone's message with .link <optional text>\
\nUsage: Generate a permanent link to the user's profile with optional custom text.\
\n\n`{xcm}regexninja` on/off\
...
\n\n`{xcm}chatinfo [optional: <reply/tag/chat id/invite link>]`\
