...
@bot.on(xubot_cmd(outgoing=True, pattern=r"filter (.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"stop (.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="rmbotfilters (.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="filters$"))
...
CMD_HELP.update({
    ...
    \n\n`{xcm}filter` <keyword> <reply text> or reply to a message with .filter <keyword>\
