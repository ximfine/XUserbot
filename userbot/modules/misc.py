...
@bot.on(xubot_cmd(outgoing=True, pattern=r"random"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"sleep ([0-9]+)$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"shutdown$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"restart$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"readme$"))
async def reedme(e):
    await e.edit(
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"repeat (.*)"))
...
async def repo_is_here(wannasee):
    await wannasee.edit(
        "[Here](https://github.com/X_Newbie/XUserbot) is XUSERBOT 🔥 Repo."
    )


@bot.on(xubot_cmd(outgoing=True, pattern=r"raw$"))
async def raw(event):
...
CMD_HELP.update(
    {
        "random": f">`{xcm}random <item1> <item2> ... <itemN>`"
        "\nUsage: Get a random item from the list of items.",
        "sleep": ">`{xcm}sleep <seconds>`"
        "\nUsage: Let yours snooze for a few seconds.",
        "shutdown": ">`{xcm}shutdown`"
        "\nUsage: Shutdown bot",
        "repo": ">`{xcm}repo`"
        "\nUsage: Github Repo of this bot",
        "readme": ">`{xcm}readme`"
        "\nUsage: Provide links to setup the userbot and it's modules.",
        "repeat": ">`{xcm}repeat <no> <text>`"
