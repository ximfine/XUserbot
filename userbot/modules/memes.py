...
@bot.on(xubot_cmd(outgoing=True, pattern="^.fuck$"))
async def iqless(e):
    await e.edit("🖕🖕🖕🖕🖕🖕🖕🖕\n🖕🖕🖕🖕🖕🖕🖕🖕\n🖕🖕\n🖕🖕\n🖕🖕\n🖕🖕🖕🖕🖕🖕\n🖕🖕🖕🖕🖕🖕\n🖕🖕\n🖕🖕\n🖕🖕\n🖕🖕\n🖕🖕")
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.rain$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.love$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.earth$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.emo$"))
async def earth(event):
    deq = deque(list("🙂😁😄😃😂🤣😭🐵🙊🙉🙈"))
    try:
        for x in range(32):
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.koc$"))
async def koc(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("8✊===D")
        await e.edit("8=✊==D")
...
@bot.on(xubot_cmd(outgoing=True, pattern="^.gas$"))
async def gas(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
...
CMD_HELP.update({
    ...
    f"\n\n>`{xcm}decide [Alternates: (.yes, .no, .maybe)]`"
    "\nUsage: Make a quick decision."
    f"\n\n>`{xcm}scam <action> <time>`"
    "\n[Available Actions: (typing, contact, game, location, voice, round, video, photo, document, cancel)]"
    "\nUsage: Create fake chat actions, for fun. (Default action: typing)"
    "\n\nAnd many more"
    f"`\n>{xcm}nou ; {xcm}bot ; {xcm}gey ; {xcm}tf ; {xcm}paw ; {xcm}taco ; {xcm}nih ;`"
