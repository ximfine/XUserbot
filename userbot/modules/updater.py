...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit('`[HEROKU]:'
                         '\nUpdate Deploy XUSERBOT, Silahkan Tunggu 5-8 menit..`'
                         )
        ups_rem.fetch(ac_br)
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('**XUSERBOT** Berhasil Di Update✓')
    await asyncio.sleep(1)
    await event.edit('`XUSERBOT Di Restart....`')
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if conf is None and force_update is False:
        changelog_str = f'**UPDATE Terbaru Untuk 🔥XUSERBOT🔥 [{ac_br}]:\n\nPERUBAHAN:**\n`{changelog}`'
    ...
    if force_update:
        ...
        await event.edit('`Proses Update XUSERBOT, ....🛠️`')
        await event.edit('`Proses Update XUSERBOT, loading....🛠️`')
        await event.edit('`Proses Update XUSERBOT, updating....🛠️`')
        await event.edit('`Proses Update XUSERBOT\nSilahkan tunggu....🛠️`')
    if conf == "now":
