        ...
        if not apprv and event.text != UNAPPROVED_MSG:
            if event.chat_id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    await event.reply.photo(f"{ALIVE_LOGO}, {UNAPPROVED_MSG}")
            else:
                await event.reply_photo(f"{ALIVE_LOGO}, {UNAPPROVED_MSG}")
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"notifoff$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"notifon$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"(?:approve|ok)\s?(.)?"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"(?:disapprove|nopm)\s?(.)?"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"block$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"unblock$"))
