...
@bot.on(xubot_cmd(outgoing=True, pattern="reserved$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="name"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="setpfp$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="setbio (.*)"))
...
@bot.on(xubot_cmd(outgoing=True, pattern="count$"))
...
@bot.on(xubot_cmd(outgoing=True, pattern=r"delpfp"))
...
@bot.on(xubot_cmd(pattern="data(?: |$)(.*)", outgoing=True))
async def who(event):
...
CMD_HELP.update({
    "profile":
    f"`{xcm}username` <new_username>\
\nUsage: Changes your Telegram username.\
\n\n`{xcm}name` <firstname> or `.name` <firstname> <lastname>\
...
\n\n`{xcm}delpfp` or `.delpfp` <number>/<all>\
...
\n\n`{xcm}data` <username> or reply to someones text with `.data`\
